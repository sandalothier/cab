...
import com.cab.tva.config.KafkaProperties;
...
class CabinetKafkaResourceIT {
    ...
    void setup() {
        ...
        CabinetKafkaResource kafkaResource = new CabinetKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/cabinet-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/cabinet-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
